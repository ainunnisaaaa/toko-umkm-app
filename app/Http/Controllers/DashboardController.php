...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $cacheKey = "dashboard_data_{$role}_{$user->id}";

        if ($request->has('refresh')) {
            \Illuminate\Support\Facades\Cache::forget($cacheKey);
            return redirect()->route('dashboard')->with('status', 'Data berhasil diperbarui.');
        }

        $data = \Illuminate\Support\Facades\Cache::remember($cacheKey, now()->addMinutes(10), function () use ($user, $role) {
            $data = [];
