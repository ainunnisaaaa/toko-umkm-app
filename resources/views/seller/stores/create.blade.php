        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('seller.stores.store') }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            <label for="logo" class="block text-sm font-medium text-gray-700">Logo URL (Opsional)</label>
                            <input type="text" name="logo" id="logo" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('logo') }}">
                            @error('logo')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('seller.stores.index') }}" class="text-gray-600 hover:text-gray-900 mr-4">Batal</a>
